...
| nReturned | 345 | 345 |
| **totalDocsExamined** | **25309** | **345** |
| executionTimeMillis | 9 | 1 |

Le coût devient proportionnel au résultat et non à la collection.

**B2 — géospatial**
```js
db.restaurants.createIndex({ "address.coord": "2dsphere" })
db.restaurants.find({ "address.coord": { $near: {
  $geometry: { type: "Point", coordinates: [-73.9857, 40.7484] }, $maxDistance: 500 } } })
```
**413 restaurants** à moins de 500 m de l'Empire State Building.
`countDocuments()` rejette `$near` — utiliser `find(q).toArray().length`.
